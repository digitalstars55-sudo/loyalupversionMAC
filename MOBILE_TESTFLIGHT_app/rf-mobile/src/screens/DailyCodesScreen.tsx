...
export const DailyCodesScreen: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const onGenerate = (branchId: number, branchName: string, purpose: DailyCodePurpose) => {
    ...
    Alert.alert(
      'Сгенерировать код?',
      `«${branchName}» — ${PURPOSE_LABELS[purpose]}.\n\nИспользуйте только если автогенерация не сработала — коды приходят автоматически в 03:00.`,
      [
      ...
      >
        ...
        <View style={[s.modalHint, { marginHorizontal: r.pad, marginBottom: 14 }]}>
          ...
          <Text style={s.modalHintText}>
            Коды дня — 5-значные коды, которые персонал сообщает гостям для активации подарков. Три вида: «День рождения», «Игра / Суперприз», «Квест». Обновляются автоматически каждую ночь в 03:00 и действуют до 03:00 следующего дня (одна рабочая ночь — один код).
          </Text>
        </View>
